...
{
    public static readonly Guid CommandSet = new("b6c7d8e9-9c5e-4f02-8d3b-7e1c9a0f6b55");
    ...
    private static async Task CreateAppAsync(AsyncPackage package)
    {
        ...
        var parent = Prompts.AskFolder("Create the Uno MVVMExpress app in this folder");
        if (parent is null)
        {
            return;
        }
    ...
    private static async Task AddPageAsync(AsyncPackage package)
    {
        ...
        var outputDir = Prompts.AskFolder("Add the Uno MVVMExpress page in this folder", startFolder);
