...
public sealed class TemplateSmokeTests
{
    ...
    public void Template_ContainsExpectedFiles()
    {
        ...
        var template = Path.Combine(root, "templates", "uno-app");
    ...
    {
        var root = FindRepoRoot();
        var source = Path.Combine(root, "templates", "uno-app");
    ...
    private static string ReplacePackages(string csproj, string projectDir, string repoRoot)
    {
        ...
        csproj = Regex.Replace(
            csproj,
            """\s*<PackageReference Include="Plugin\.Uno\.MVVMExpress\.Core" Version="\$\(UnoMvvmExpressVersion\)" />""",
            $"""{Environment.NewLine}    <ProjectReference Include="{Ref("Plugin.Uno.MVVMExpress.Core")}" />""");
        csproj = Regex.Replace(
            csproj,
            """\s*<PackageReference Include="Plugin\.Uno\.MVVMExpress\.Pagination" Version="\$\(UnoMvvmExpressVersion\)" />""",
            $"""{Environment.NewLine}    <ProjectReference Include="{Ref("Plugin.Uno.MVVMExpress.Pagination")}" />""");
        csproj = Regex.Replace(
            csproj,
            """\s*<PackageReference Include="Plugin\.Uno\.MVVMExpress\.Testing" Version="\$\(UnoMvvmExpressVersion\)" />""",
            $"""{Environment.NewLine}    <ProjectReference Include="{Ref("Plugin.Uno.MVVMExpress.Testing")}" />""");
        return csproj;
    }
