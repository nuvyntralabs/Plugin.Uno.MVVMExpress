...
public sealed class ModuleAndAdapterTests
{
    ...
    private sealed class MemoryAuth : IAuthState
    {
        ...
        public Task<Plugin.Uno.MVVMExpress.Outcome.Outcome> SignInAsync(string userName, string password, CancellationToken cancellationToken = default)
        {
            SignedIn = true;
            return Task.FromResult(Plugin.Uno.MVVMExpress.Outcome.Outcome.Success());
        }
