...
def collect_library(plugin_root: Path, product: str) -> dict[str, str]:
    ...
    template_props = plugin_root / "templates/uno-app/Directory.Build.props"
    template_version = first_property(template_props, "UnoMvvmExpressVersion")
    if not template_version:
        fail(f"{template_props} has no UnoMvvmExpressVersion")
    versions[str(template_props.relative_to(plugin_root)) + " UnoMvvmExpressVersion"] = template_version
    return versions
...
def collect_extensions(plugin_root: Path) -> dict[str, str]:
    return {
        ...
        "extensions/visualstudio/UnoMvvmExpressPackage.cs": read_regex(
            plugin_root / "extensions/visualstudio/UnoMvvmExpressPackage.cs",
            r'InstalledProductRegistration\("Uno MVVMExpress",\s*"[^"]+",\s*"([^"]+)"\)',
        ),
    }
