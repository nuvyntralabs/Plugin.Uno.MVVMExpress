...
using Plugin.Uno.MVVMExpress.Lifecycle;
using Plugin.Uno.MVVMExpress.Threading;
using Result = Plugin.Uno.MVVMExpress.Outcome.Outcome;
...
/// </summary>
public sealed class UnoFrameNavigator : IPageNavigator, IRouteResolver
{
    ...
    public UnoFrameNavigator Map(Type viewModelType, Type viewType, string? route = null)
    {
    ...
    private async Task<Result> GoOnUiAsync(
        ...
        CancellationToken cancellationToken)
    {
        var currentView = UnoVisualTree.CurrentView(Window);
    ...
    private Frame? ResolveFrame() => _frame is not null ? _frame() : UnoVisualTree.CurrentFrame(Window);
