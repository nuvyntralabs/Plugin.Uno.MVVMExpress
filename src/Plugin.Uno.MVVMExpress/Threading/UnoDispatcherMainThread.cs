...
namespace Plugin.Uno.MVVMExpress.Threading;

/// <summary>WinUI <see cref="DispatcherQueue"/> adapter for <see cref="IMainThread"/>.</summary>
public sealed class UnoDispatcherMainThread : IMainThread
{
    private readonly DispatcherQueue _queue;

    public UnoDispatcherMainThread()
        : this(DispatcherQueue.GetForCurrentThread() ?? DispatcherQueue.GetForCurrentThread())
    {
    }

    public UnoDispatcherMainThread(DispatcherQueue queue)
    {
